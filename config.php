<?php
$DBHOST = getenv('db4free.net'); // Set your database host here
$DBUSER = getenv('toando249'); // Set your database user here
$DBNAME = getenv('toando249'); // Set your database name here
$DBPW =  getenv('toan12345678'); // Set your database password here
$BOT_ID = getenv('5f49cae901c345658fcb4963');
$TOKEN = getenv('U1ZdYozpo1V8Mry3nmUss1uP6aTFDU6Z2gc3EcVCcwSW7BXwZfngq9HXfle7XaDb');

$NUDE_API_USER = getenv('NUDE_API_USER');
$NUDE_API_SECRET = getenv('NUDE_API_SECRET');

$BLOCK_NAME = "uet_chat";
$JSON = "uet_text";
$BLOCK_IMAGE = "uet_image";
$BLOCK_VOID = "uet_void";
$BLOCK_VIDEO = "uet_video";
$BLOCK_FILE = "uet_file";

?>
